...
router.post('/chat/completions', async (req, res) => {
    ...
    // Log the FULL incoming request shape so we can see what ElevenLabs sends
    // (especially whether it includes tool definitions we need to honor)
    console.log(`[CustomLLM] REQ keys=${Object.keys(req.body || {}).join(',')} stream=${stream} model=${model} hasTools=${!!tools} toolCount=${tools?.length || 0} tool_choice=${tool_choice} msgCount=${messages?.length || 0}`);
    if (tools?.length) {
        console.log(`[CustomLLM] tools=${JSON.stringify(tools.map((t) => t.function?.name || t.name))}`);
    }
